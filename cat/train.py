...
def run_training(args, train_config, data_config, lora_config):
    ...
    # ============================================================
    # Hint 源选择: gt / cat / mixed
    # ============================================================
    hint_dict_all = None
    hint_source = args.hint_mode

    if hint_source in ("cat", "mixed"):
    ...
    # 训练 collator
    # hint_dropout 策略:
    #   gt:    0.3 (需要学到"无 hint 时 fallback")
    #   cat:   0.0 (严格对齐)
    #   mixed: 0.1 (轻度 robustness)
    dropout_map = {"gt": 0.3, "cat": 0.0, "mixed": 0.1}
