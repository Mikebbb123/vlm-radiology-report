...
def generate_single(
    ...
    # 优先级: CAT hint > CheXNet > 无 hint (VFF 和 hint 独立, 可以组合)
    ...
    # VFF: 注入 medical feature 和 grid_thw
...
def main():
    ...
    # VFF 模式自动把 model_path 切到 lora_vff (除非用户显式指定)
    ...
    if args.use_vff:
        ...
        mode = f"CheXNet (threshold={args.threshold})" if disease_classifier else "Baseline"
    else:
