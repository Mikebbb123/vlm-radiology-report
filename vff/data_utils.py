...
class DualViewCollator:
    ...
    def __init__(self, processor,
        ...
        self.densenet_feat_dict = densenet_feat_dict  # {sample_id: np.ndarray [1024]}
    ...
    def __call__(self, features: List[dict]) -> dict:
        ...
        # VFF: 如果提供了 densenet_feat_dict, 把每个样本的 feature stack 到 batch
        if self.densenet_feat_dict is not None:
            import numpy as np
            feats = []
            for f in features:
                sid = f.get("id", "")
                feat = self.densenet_feat_dict.get(sid)
                if feat is None:
                    # 样本没 feature: 用零向量 (VFF 的 gate 初始为 0, 不影响训练)
                    feat = np.zeros(1024, dtype=np.float32)
                feats.append(feat)
            inputs["medical_feats"] = torch.from_numpy(np.stack(feats)).float()
...
# ============================================================
# 9. DenseNet feature 加载 (Visual Feature Fusion)
# ============================================================

def load_densenet_feats(feat_file: str) -> dict:
    """
    加载 .npz 里的 DenseNet penultimate features.
    返回 {"train": {id: np.ndarray [1024]}, "val": {...}, "test": {...}}
    """
    import numpy as np
    if not os.path.exists(feat_file):
        raise FileNotFoundError(
            f"DenseNet feature file not found: {feat_file}\n"
            f"Run: python precompute_densenet_features.py"
        )
    ...
    for split in ["train", "val", "test"]:
        ...
        result[split] = {str(sid): feats[i] for i, sid in enumerate(ids)}
        print(f"[VFF] Loaded {len(result[split])} {split} features "
              f"(dim={feats.shape[1] if len(feats) > 0 else 'N/A'})")
    return result
